...
exports.scheduledAllocation = scheduledAllocation;
...
function enrichFoodDonationsWithCoordinates(foodDonations) {
        ...
        for (const donation of foodDonations) {
            try {
                ...
                if (donation.latitude !== undefined && donation.longitude !== undefined && donation.pickup_location) {
                    enrichedDonations.push(donation);
                    continue;
                }
                ...
                const coordinates = yield mapService_1.default.getCoordinates(donation.pickup_location);
                // Create enriched donation object with coordinates
                enrichedDonations.push(Object.assign(Object.assign({}, donation), { latitude: coordinates.lat, longitude: coordinates.lng }));
            }
            catch (error) {
...
function allocateFoodWithORTools(ngos, foodDonations, previousAllocations) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
...
router.post('/allocate', auth_1.authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const user = req.user;
        ...
        const ngoQuery = yield (0, util_1.query)(`SELECT 
       ...
       AND expiration_time > CURRENT_TIMESTAMP`, []);
        ...
        const previousAllocationsQuery = yield (0, util_1.query)(`SELECT 
       ...
       WHERE allocation_status = 'COMPLETED'`, []);
        ...
        const enrichedFoodDonations = yield enrichFoodDonationsWithCoordinates(foodQuery.rows);
        // Run the OR-Tools allocation algorithm
        const allocations = yield allocateFoodWithORTools(ngoQuery.rows, enrichedFoodDonations, previousAllocationsQuery.rows);
        // Store the new allocations in the database
        // First, delete any pending allocations
        yield (0, util_1.query)(`DELETE FROM food_allocations 
       WHERE allocation_status = 'PENDING'`, []);
        // Insert new allocations
        for (const allocation of allocations) {
            yield (0, util_1.query)(`INSERT INTO food_allocations 
         (food_donation_id, ngo_id, allocated_quantity, allocation_status)
         VALUES ($1, $2, $3, 'PENDING')`, [allocation.food_donation_id, allocation.ngo_id, allocation.allocated_quantity]);
        }
...
router.get('/allocations/user', auth_1.authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const user = req.user;
        if (!(user === null || user === void 0 ? void 0 : user.id)) {
            res.status(401).json({
        ...
        const ngoQuery = yield (0, util_1.query)(`SELECT id FROM ngos WHERE user_id = $1`, [user.id]);
        ...
        const allocationsQuery = yield (0, util_1.query)(`SELECT 
       ...
       WHERE a.ngo_id = $1
       ORDER BY f.expiration_time ASC`, [ngoId]);
        res.status(200).json({
            success: true,
...
router.put('/allocations/:allocationId', auth_1.authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const user = req.user;
        ...
        const allocationAccess = yield (0, util_1.query)(`SELECT a.id, a.ngo_id, a.food_donation_id, a.allocated_quantity
       FROM food_allocations a
       JOIN ngos n ON a.ngo_id = n.id
       WHERE a.id = $1 AND n.user_id = $2`, [allocationId, user.id]);
        ...
        if (action === 'ACCEPT') {
            ...
            const foodQuery = yield (0, util_1.query)(`SELECT id, COALESCE(weight_kg, servings, quantity) as quantity
         FROM food_donations
         WHERE id = $1 AND status = 'AVAILABLE'`, [allocation.food_donation_id]);
            ...
            yield (0, util_1.query)(`UPDATE food_allocations 
         ...
         SET quantity = $1
         WHERE id = $2`, [newQuantity, allocation.food_donation_id]);
            // If quantity is now 0, update the status to 'ALLOCATED'
            if (newQuantity <= 0) {
                yield (0, util_1.query)(`UPDATE food_donations 
           SET status = 'ALLOCATED' 
           WHERE id = $1`, [allocation.food_donation_id]);
            }
            res.status(200).json({
                success: true,
            ...
            yield (0, util_1.query)(`DELETE FROM food_allocations WHERE id = $1`, [allocationId]);
            res.status(200).json({
                success: true,
...
router.get('/allocations', auth_1.authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const user = req.user;
        // Verify user is an admin
        if (!(user === null || user === void 0 ? void 0 : user.id) || user.role.toUpperCase() !== 'ADMIN') {
            res.status(403).json({
                success: false,
...
router.post('/manual-allocate', auth_1.authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        const user = req.user;
        // Verify user is an admin
        ...
        const foodQuery = yield (0, util_1.query)(`SELECT id, remaining_quantity, status
       FROM food_donations
       WHERE id = $1 AND status IN ('AVAILABLE', 'PARTIALLY_ALLOCATED')`, [food_donation_id]);
        if (!foodQuery.rows.length) {
            res.status(404).json({
                success: false,
        ...
        const ngoQuery = yield (0, util_1.query)(`SELECT id, storage_capacity_kg
       FROM ngos
       WHERE id = $1 AND is_verified = TRUE `, [ngo_id]);
        if (!ngoQuery.rows.length) {
            res.status(404).json({
                success: false,
        ...
        const currentAllocationsQuery = yield (0, util_1.query)(`SELECT SUM(allocated_quantity) as total_allocated
       FROM food_allocations
       WHERE ngo_id = $1 AND allocation_status IN ('PENDING', 'ACCEPTED')`, [ngo_id]);
        const currentAllocated = parseFloat(((_a = currentAllocationsQuery.rows[0]) === null || _a === void 0 ? void 0 : _a.total_allocated) || '0');
        ...
        const insertQuery = yield (0, util_1.query)(`INSERT INTO food_allocations 
       (food_donation_id, ngo_id, allocated_quantity, allocation_status)
       VALUES ($1, $2, $3, 'PENDING')
       RETURNING id`, [food_donation_id, ngo_id, quantity]);
        res.status(201).json({
            success: true,
